...
public class Q1846 {
    ...
    // 方法2：计数排序
    // 统计每个数字出现的次数，并统计缺失的数字个数，用后边多余的数字补全前边缺失的数字
    // 最后n减去缺失的个数，即为可补全的最大数值
    public int maximumElementAfterDecrementingAndRearranging(int[] arr) {
        int[] temp = new int[arr.length + 1];
        for (int i : arr) {
            // 将大于n的数字当作n计数
            temp[Math.min(i, arr.length)]++;
        }
        int miss = 0;
        for (int i = 1; i < temp.length; i++) {
            if (temp[i] == 0) {
                ...
                // 只能补全前边缺失的，不能向后补全，因为数字只能减小不能增大
                miss -= Math.min(temp[i] - 1, miss);
            }
        }
